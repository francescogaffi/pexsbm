...
The documentation is organized in two main folders (with multiple subfolders) and one source file described below.

- `pex-sbm.R`. It contains all the s**source** `R` **functions** which are required to perform posterior computation, inference and prediction under the **pEx-SBM** class (with a particular focus on the three examples studied in the article: H-DP, H-NSP and H-DP with hyperprior).
...
- `SIMULATIONS`. It contains three subfolders to reproduce the results for the **simulation studies in Section 4**:
	...
	- `ROBUSTNESS`: Contains three subfolders with codes to reproduce the robustness analyses in Section S4.2 (Supplementary Material).
