...
# fit pEx-SBM(HNSP)
set.seed(r)
model_rem <- pex_sbm(Yrem,n_iter,Nrem,1,1,sigma=sigma, sigma0=sigma0, prior="HNSP")
z_post    <- model_rem[[1]]
w_post    <- model_rem[[2]]
hpars     <- as.numeric(model_rem[[3]][4:5])
...
for (i in 1:n_mc){
  ...
  for(ind1 in 1:m){
    ...
    for(ind2 in 1:Vrem){
      P_old[ind1,ind2] <- coclust_old(l,q,remlay[ind1],z_post[ind2,n],NA,NA,hpars[1],hpars[2],"HNSP",Nrem)
    }
    # compute co-clustering probabilities among new nodes
    if(ind1>1)
      for(ind3 in 1:(ind1-1)){
        P_new[ind1,ind3] <- coclust_new(l,q,remlay[ind1],remlay[ind3],NA,NA,hpars[1],hpars[2],"HNSP",Nrem)
      }
  }
...
# edge prediction
set.seed(r)
pred      <- pred_conn(Yrem,layrem,remlay,model_rem,n_mc)
Ynew      <- pred[[1]]
...
save(waic, vi, expected_vi, vi_bound, z_estimated, H_estimated, H_posterior, misclass, pred_prob_err, file="output_HNSP.RData")
