...
set.seed(r)
model1       <- pex_sbm(Y1,n_iter,N,1,1,theta,theta0,z_init = rep(1,V))
z_post1      <- model1[[1]]
...
set.seed(r)
model2       <- pex_sbm(Y2,n_iter,N,1,1,theta,theta0,z_init = rep(1,V))
z_post2      <- model2[[1]]
...
save(expected_vi,  file="output_HDP.RData")
