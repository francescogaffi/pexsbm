...
getCascSvd = function(graphMat, covariates, hTuningParam, nBlocks, assortative) {
    ...
    sDecomp = irlba(A = graphMat, nu = nBlocks + 1, nv = 0,work = 50,
        mult = matMult,fastpath=FALSE)
...
getTuningRange = function(graphMatrix, covariates, nBlocks,
    ...
    if(assortative == T) {
        ...
        eigenValGraph = irlba(graphMatrix, nu = nBlocks + 1, nv = 0,work = 50,fastpath=FALSE)$d
        ...
        eigenValGraph = (irlba(graphMatrix, nu = nBlocks + 1, nv = 0,work = 50,fastpath=FALSE)$d)^2
